  ...
  describe('Login Integration', () => {
    it('should update store and persist state after successful login', async () => {
      ...
      await waitFor(() => {
        ...
        expect(result.current.store.accessToken).toBe('test-access-token');
        expect(result.current.store.refreshToken).toBe('test-refresh-token');
      });

      // Hook should reflect store state
      expect(result.current.auth.isAuthenticated).toBe(true);
      expect(result.current.auth.user).toEqual(mockUser);
      expect(result.current.auth.token).toBe('test-access-token');
    });
  ...
  describe('Logout Integration', () => {
    it('should clear store after logout', async () => {
      ...
      await waitFor(() => {
        ...
        expect(result.current.store.refreshToken).toBeNull();
      });
