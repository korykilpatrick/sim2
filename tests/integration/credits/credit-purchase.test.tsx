...
describe('Credit Purchase Integration Tests', () => {
  ...
  describe('Purchase Flow', () => {
    it('should complete purchase successfully', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText(/Purchase 500 Credits/i),
    ...
    it('should show transaction in history after purchase', async () => {
      ...
      expect(
        within(transactionHistory).getByText(/\+100 credits/i),
      ).toBeInTheDocument()
      expect(
        within(transactionHistory).getByText(/Credit purchase/i),
      ).toBeInTheDocument()
    })
  })
  ...
  describe('Payment Processing', () => {
    it('should handle payment method selection', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('Mastercard ending in 5678'),
        ).toBeInTheDocument()
  ...
  describe('Confirmation and Receipt', () => {
    it('should show purchase confirmation with details', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          within(confirmation).getByText('1,000 credits'),
        ).toBeInTheDocument()
        expect(within(confirmation).getByText('$80.00')).toBeInTheDocument()
        expect(
          within(confirmation).getByText(/Transaction ID:/i),
        ).toBeInTheDocument()
      })
    })
