...
describe('Credit Deduction Integration Tests', () => {
  describe('Vessel Tracking Credit Deduction', () => {
    it('should deduct credits when starting vessel tracking', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Tracking started successfully/i),
        ).toBeInTheDocument()
      })
  ...
  describe('Area Monitoring Credit Deduction', () => {
    it('should calculate and deduct credits based on area size', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Area created successfully/i),
        ).toBeInTheDocument()
      })
  ...
  describe('Credit Reservation Flow', () => {
    it('should reserve credits before confirming transaction', async () => {
      ...
      // Reserve credits
      const reservation = await result.current.reserveCredits(
        100,
        'vessel-tracking',
      )
      ...
      // Confirm reservation
      const confirmResult = await result.current.confirmReservation(
    ...
    it('should cancel reservation on failure', async () => {
      ...
      // Cancel reservation
      const cancelResult = await result.current.cancelReservation(
        reservation.reservationId,
      )
