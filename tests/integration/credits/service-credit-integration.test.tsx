...
describe('Service-Specific Credit Integration', () => {
  ...
  describe('Investigation Credits', () => {
    it('should show investigation costs upfront', async () => {
      ...
      const newInvestigationBtn = await screen.findByText(/Request Investigation/i)
      ...
      await waitFor(() => {
        ...
        const comprehensiveOption = screen.getByTestId('investigation-comprehensive')
        
        expect(within(basicOption).getByText(/5,000 credits/i)).toBeInTheDocument()
        expect(within(comprehensiveOption).getByText(/10,000 credits/i)).toBeInTheDocument()
      })
    })
    ...
    it('should handle large credit deductions for investigations', async () => {
      ...
      await waitFor(() => {
        ...
        expect(within(confirmDialog).getByText(/Confirm 5,000 credit deduction/i)).toBeInTheDocument()
      })
      ...
      // Wait for completion
      await waitFor(() => {
        expect(screen.getByText(/Investigation requested successfully/i)).toBeInTheDocument()
      })
    ...
    it('should show credit package suggestions for expensive investigations', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/You need 8,000 more credits/i)).toBeInTheDocument()
        expect(screen.getByText(/Suggested credit packages:/i)).toBeInTheDocument()
      ...
      expect(purchaseLink).toHaveAttribute('href', '/credits?purchase=true&amount=5000')
    })
  })
  ...
  describe('Fleet Tracking Credits', () => {
    it('should calculate fleet tracking costs based on vessel count', async () => {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        const vesselInput = screen.getAllByPlaceholderText(/Search vessels/i).at(-1)!
      ...
      await waitFor(() => {
        ...
        expect(within(costBreakdown).getByText(/15 vessels/i)).toBeInTheDocument()
        expect(within(costBreakdown).getByText(/100 credits per vessel per month/i)).toBeInTheDocument()
        expect(within(costBreakdown).getByText(/3 months/i)).toBeInTheDocument()
        expect(within(costBreakdown).getByText(/Subtotal: 4,500 credits/i)).toBeInTheDocument()
        expect(within(costBreakdown).getByText(/No bulk discount/i)).toBeInTheDocument() // 15 vessels = no discount
        expect(within(costBreakdown).getByText(/Total: 4,500 credits/i)).toBeInTheDocument()
      })
    })
    ...
    it('should apply bulk discounts for large fleets', async () => {
      ...
      await waitFor(() => {
        const costBreakdown = screen.getByTestId('fleet-cost-breakdown')
        expect(within(costBreakdown).getByText(/30 vessels/i)).toBeInTheDocument()
        expect(within(costBreakdown).getByText(/10% bulk discount/i)).toBeInTheDocument()
        expect(within(costBreakdown).getByText(/Subtotal: 3,000 credits/i)).toBeInTheDocument()
        expect(within(costBreakdown).getByText(/Discount: -300 credits/i)).toBeInTheDocument()
        expect(within(costBreakdown).getByText(/Total: 2,700 credits/i)).toBeInTheDocument()
      })
    })
  })
  ...
  describe('Credit Warnings Across Services', () => {
    it('should show consistent low balance warnings', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/Low credit balance: 45 credits/i)).toBeInTheDocument()
      })
      unmount1()
