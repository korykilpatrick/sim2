  ...
  describe('Transaction Display', () => {
    it('should display transaction history', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/Vessel tracking - 10 days/i),
        ).toBeInTheDocument()
    ...
    it('should show transaction types with appropriate styling', async () => {
      ...
      await waitFor(() => {
        ...
        expect(within(purchaseRow!).getByText(/\+500/)).toHaveClass(
          'text-green-600',
        )

        // Deduction should have red styling
        expect(within(deductionRow!).getByText(/-50/)).toHaveClass(
          'text-red-600',
        )
      })
    })
  ...
  describe('Filtering and Search', () => {
    it('should filter transactions by type', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.queryByText(/Credit package purchase/i),
  ...
  describe('Empty States', () => {
    it('should show empty state when no transactions', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/Your credit transactions will appear here/i),
        ).toBeInTheDocument()
      })
    })
  })
  ...
  describe('Transaction Details', () => {
    it('should show transaction details on click', async () => {
      ...
      const transactionRow = screen
        .getByText(/Credit package purchase/i)
        .closest('tr')
      ...
      await waitFor(() => {
        ...
        expect(
          within(modal).getByText(/Transaction Details/i),
        ).toBeInTheDocument()
        expect(within(modal).getByText(/Transaction ID:/i)).toBeInTheDocument()
        expect(within(modal).getByText('tx-1')).toBeInTheDocument()
        expect(
          within(modal).getByText(/Amount: \+500 credits/i),
        ).toBeInTheDocument()
        expect(
          within(modal).getByText(/New Balance: 1,500 credits/i),
        ).toBeInTheDocument()
      })
    })
  })
