    ...
    it('should catch invalid area type', () => {
      const invalidResponse = {
        ...
        data: {
          id: 'a1',
          name: 'Test Area',
          type: 'zone', // Invalid type (not in enum)
      ...
      const validateCreditBalance = (data: unknown) => {
        try {
          ...
          if (error instanceof ApiValidationError) {
            ...
            console.error('Credit balance validation failed:', error.validationErrors)
      ...
      const validateReport = (data: unknown) => {
        try {
          ...
          if (error instanceof ApiValidationError) {
            ...
            throw new Error(`Received incomplete report data. Missing fields: ${missingFields.join(', ')}`)
          }
          throw error
        }
      }

      expect(() => validateReport(incompleteResponse)).toThrow(/Received incomplete report data/)
    })
