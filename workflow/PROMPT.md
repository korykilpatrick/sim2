...
### 3.1 Visual Testing Requirements

For ANY frontend changes:

1. **Before marking complete**, use Puppeteer to:

   - Navigate to the affected page(s)
   - Take screenshots of all states (initial, loading, success, error)
...
2. **Visual Testing Checklist**:
   ...
   - [ ] Accessibility features work (focus states, ARIA)
...
##### Pass 8: Visual Verification (Frontend Changes Only)

- Use Puppeteer to navigate to ALL affected pages
- Take screenshots documenting the changes
- Verify no visual regressions in other areas
- Confirm all user interactions work as expected
- Check browser console is clean
- Test in different viewport sizes if responsive
