...
export const areaKeys = {
  ...
  alerts: (id: string, filters?: { severity?: 'low' | 'medium' | 'high' | 'critical'; type?: string; isRead?: boolean; limit?: number; page?: number }) => [...areaKeys.all, 'alerts', id, filters] as const,
  statistics: () => [...areaKeys.all, 'statistics'] as const,
  criteria: () => [...areaKeys.all, 'monitoring-criteria'] as const,
  cost: (config: { sizeKm2: number; criteria: string[]; updateFrequency: number; duration: number }) => [...areaKeys.all, 'cost', config] as const,
