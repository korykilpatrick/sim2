...
import { formatInputDate, addDuration } from '@/lib/date';
...
export function TrackingWizard({ onComplete, initialData }: TrackingWizardProps) {
  const [formData, setFormData] = useState<TrackingWizardData>({
    ...
    endDate: initialData?.endDate || formatInputDate(addDuration(new Date(), 30, 'days')),
  });

  const wizard = useWizard({
    steps: [
      { id: 'vessel', label: 'Select Vessel', description: 'Choose a vessel to track' },
      { id: 'criteria', label: 'Tracking Criteria', description: 'Select what to monitor' },
  ...
  const canComplete = canProceedFromVessel && canProceedFromCriteria && formData.trackingDays > 0;
        ...
        <Card>
          ...
          <CardContent>
            <CriteriaSelectionStep
              ...
              onCriteriaChange={(criteria) => setFormData({ ...formData, criteria })}
            />
          </CardContent>
        </Card>
      </WizardStep>
        ...
        <Card>
          ...
          <CardContent>
            <DurationConfigStep
              ...
              onDaysChange={(trackingDays) => setFormData({ ...formData, trackingDays })}
