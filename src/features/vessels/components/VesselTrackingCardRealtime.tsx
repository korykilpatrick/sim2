...
interface VesselTrackingCardRealtimeProps {
  tracking: VesselTracking
}

export default function VesselTrackingCardRealtime({
  tracking: initialTracking,
}: VesselTrackingCardRealtimeProps) {
  const [tracking, setTracking] = useState(initialTracking)
  const [isLive, setIsLive] = useState(false)
  const [lastUpdate, setLastUpdate] = useState<Date | null>(null)

  // Subscribe to vessel room for real-time updates
  useWebSocketRoom('vessel', tracking.vessel.id, true)

  // Handle position updates
  useWebSocketEvent('vessel_position_update', (data: VesselPositionUpdate) => {
    if (data.vesselId === tracking.vessel.id) {
      setTracking((prev) => ({
        ...prev,
        vessel: {
          ...prev.vessel,
  ...
  useWebSocketEvent('vessel_status_change', (data: VesselStatusChange) => {
    if (data.vesselId === tracking.vessel.id) {
      setTracking((prev) => ({
        ...prev,
        vessel: {
          ...prev.vessel,
          status: data.newStatus as typeof prev.vessel.status,
        },
      }))
      setLastUpdate(new Date())
    }
  ...
  // Show live indicator after 5 seconds without updates
  useEffect(() => {
    if (lastUpdate) {
        ...
        <dl className="space-y-2 text-sm">
          <div className="flex justify-between">
            ...
            <dd className="font-medium text-gray-900 flex items-center gap-2">
              ...
              {tracking.vessel.status === 'active' &&
                tracking.vessel.lastPosition.speed &&
                tracking.vessel.lastPosition.speed > 1 && (
                  <svg
                    className="h-3 w-3 text-green-500 animate-pulse"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path d="M10 18a8 8 0 100-16 8 8 0 000 16zM9 5v6l4 2" />
                  </svg>
                )}
            </dd>
          </div>
          <div className="flex justify-between">
            ...
            <dd className="font-medium text-gray-900">
              {tracking.vessel.lastPosition.lat.toFixed(4)}°,{' '}
              {tracking.vessel.lastPosition.lng.toFixed(4)}°
            </dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-gray-500">Speed:</dt>
            <dd className="font-medium text-gray-900">
              {tracking.vessel.lastPosition.speed?.toFixed(1) || 0} knots
            </dd>
          </div>
          <div className="flex justify-between">
