...
const TestComponent = () => {
  return (
    <div data-testid="test-component">WebSocket Test Component</div>
  );
};
...
describe('WebSocketProvider', () => {
  ...
  describe('Initialization', () => {
    ...
    it('should connect when authenticated on mount', async () => {
      ...
      await waitFor(() => {
        expect(websocketService.connect).toHaveBeenCalledWith('mock-access-token');
      });
    });
  ...
  describe('Event Listeners', () => {
    it('should register connection event listeners', async () => {
      ...
      await waitFor(() => {
        expect(websocketService.on).toHaveBeenCalledWith('connect', expect.any(Function));
        expect(websocketService.on).toHaveBeenCalledWith('disconnect', expect.any(Function));
        expect(websocketService.on).toHaveBeenCalledWith('authenticated', expect.any(Function));
        expect(websocketService.on).toHaveBeenCalledWith('unauthorized', expect.any(Function));
      });
    });
    ...
    it('should handle authenticated event', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[WebSocketProvider] Authenticated:', true);
    ...
    it('should handle unauthorized event', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[WebSocketProvider] Unauthorized:', 'Invalid token');
  ...
  describe('Debug Mode', () => {
    it('should register debug listeners when enabled', async () => {
      ...
      await waitFor(() => {
        expect(websocketService.on).toHaveBeenCalledWith('server_message', expect.any(Function));
        expect(websocketService.on).toHaveBeenCalledWith('maintenance_mode', expect.any(Function));
      });
    });
  });
